  ...
  def save_tiles(self):
    ...
    for i in range(4):
      ...
      if not os.path.exists(dir):
        os.makedirs(dir)
      if i == 0:
        dir_set0 = os.path.join(os.path.dirname(img_path), str(pid) + '-' + os.path.basename(img_path.replace('.png', '')))
        if not os.path.exists(dir_set0):
          os.mkdir(dir_set0)
      img_resized.save(os.path.join(dir_set0, os.path.basename(img_path).replace('c1', 'order' + str(i))))
      x_new_center = x + w/2
      y_new_center = y + h/2
      x_new_corner = x_new_center - w*scales[i]/2
      y_new_corner = y_new_center - h*scales[i]/2
      x = int(x_new_corner)
      y = int(y_new_corner)
    
    # After getting the 4 tiles, do permutation to generate 24 arrangements
    permutations = list(itertools.permutations([0, 1, 2, 3])) # 4 tiles
    tiles_set0 = sorted(glob_function(os.path.join(dir_set0, '*')))
    # If some of the 4 tiles were removed by mask percentage, then delete this set
    for tile in tiles_set0:
      black_percentage, white_percentage = my_mask_percent(np.asarray(Image.open(tile)))
      if black_percentage >= 60 or white_percentage >= 60:
        shutil.rmtree(dir_set0)
        break

    # If all 4 tiles were kept
    if os.path.exists(dir_set0):
      for i in range(len(permutations)):
        if i != 0:
          dir_this = dir_set0.replace('c1', 'c' + str(i+1))
          if not os.path.exists(dir_this):
            os.makedirs(dir_this)
          this_permutation = permutations[i]
          for t in range(len(tiles_set0)):
            shutil.copy(tiles_set0[t], dir_this)
