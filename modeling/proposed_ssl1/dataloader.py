...
class DataProcessor(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if 'no' in img_pair_dir and '20x.png' in os.path.basename(all_imgs[0]):
            label = torch.tensor(0)
        elif 'yes' in img_pair_dir and '20x.png' in os.path.basename(all_imgs[0]):
            label = torch.tensor(1)
        elif 'no' in img_pair_dir and '10x.png' in os.path.basename(all_imgs[0]):
            label = torch.tensor(0) + torch.tensor(2)
        elif 'yes' in img_pair_dir and '10x.png' in os.path.basename(all_imgs[0]):
